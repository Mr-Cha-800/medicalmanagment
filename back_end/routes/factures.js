...
Router.get('/:id/:caisse',(req,res)=>{
    if(req.params.caisse === 'CASH'){
        mysqlConnection.query('SELECT devis_berk.ID_seyed as ID_seyed, devis_berk.Nom_Prenom as Nom_prenom,  devis_berk.TTConly as TTConly, devis_berk.TVAonly as TVAonly, devis_berk.Remiseonly as Remiseonly,  devis_berk.Wilaya as Wilayaa,devis_berk.Caisse as Caissee,devis_berk.Numsecsocial as Numsecsocial,devis_berk.montant_TTC as montant_TTC,devis_berk.Cash as Cash,devis_berk.Tva as Tva, devis_berk.etat as etat, devis_berk.ID as idfact,devis_berk.Annee as factyear,devis_berk.montant_total as montants,devis_berk.remise as remise, patient_nom,patient_prenom,patient_datenaiss,patient_lieunaiss, dossier.*, achatdevis.*,produits.*, achatdevis.quantity as quantities  FROM devis_berk,dossier,achatdevis,produits WHERE dossier.ID = devis_berk.foreignID AND devis_berk.ID = achatdevis.id_fact AND devis_berk.ID = ? AND produits.NumRef = achatdevis.id_produit',[req.params.id],(err,rows,fields)=>{
            if(!err)
            res.send(rows);
            else
            console.log(err);
